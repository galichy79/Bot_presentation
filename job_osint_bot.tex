...
\title{JobOsint Bot Protect your career}
...
\begin{frame}
    ...
    \begin{block}{AI-based Telegram bot that checks employers faster}
        \begin{itemize}
            \item Find job postings of your employer on .ru and .by websites
            \item Analyze employer connections with the aggressor country 
        \end{itemize}
    \end{block}
...
\begin{frame}
    \frametitle{Money raising}
        ...
        \begin{tikzpicture}
            ...
            % Первая карточка
            \node[card] (card1) at (-6,0) {\textbf{Ready MVP} };
            
            % Вторая карточка
            \node[card] (card2) at (-3,0) {Implement \\ marketing \\ strategy};
            
            % Третья карточка
            \node[card] (card3) at (0,0) {Scale user base, optimize performance};
        ...
        \begin{tikzpicture}
            ...
            % Подписи под кругами
            \node at (-6,-2.5) {\textbf{Q3-2024}};
            \node at (-3,-2.5) {\textbf{Q4-2024}};
            \node at (0,-2.5) {\textbf{Q1-2025}};
            \node at (3,-2.5) {\textbf{Q3-2025}};
            \node at (6,-2.5) {\textbf{Q4-2027}};
            
            % Текст с чередованием цветов, размещенный ниже
            \node at (-6,-3.5) {\textbf{\textcolor{orange}{} }};
            \node at (-3,-3.5) {\textbf{first\textcolor{orange}{} users}};
...
\begin{frame}
    ...
    \begin{tikzpicture}[remember picture,overlay]
        ...
        \foreach \x in {0,...,15} {
            \draw[opacity=0.2, thin, blue!50] (\x,0) -- (\x-2,9);
        }
    
        % Текст в верхней части слайда, отцентрированный
        \node[align=center] at ([yshift=4cm]current page.center) {
        ...
        % Внешняя рамка визитки
        \draw[rounded corners=10pt, thick] 
            ([yshift=1cm,xshift=-3cm]current page.south) rectangle ([yshift=5.2cm,xshift=3cm]current page.south);

        % Текст внутри визитки
        \node[align=center, font=\sffamily] at ([yshift=4.7cm,xshift=0cm]current page.south) {\textbf{CEO | Yuriy Galych}};
        \node[align=center, font=\sffamily] at ([yshift=4.2cm,xshift=0cm]current page.south) {JobOsintBot@proton.me};

        % QR-код, размещенный снизу
        \node at ([yshift=3cm,xshift=0cm]current page.south) {\qrcode{https://www.linkedin.com/in/yuriy-galych-428068182/}};

        % Абсолютное позиционирование текста "Check employer" внизу слайда
        \node[align=center] at ([yshift=0.5cm]current page.south) {\href{https://t.me/proj_talexhub_bot}{\textcolor{blue}{\textbf{Check employer}}}};
